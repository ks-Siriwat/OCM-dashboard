...
fd.spRendered(async function () {
    ...
    const filterDataTable = (controlName) => {
        fd.control(controlName).filter = `<Eq><FieldRef Name="SF_JobCode"/><Value Type="Text">${fd.control('ProjectCode').value?.LookupValue || null}</Value></Eq>`;
        fd.control(controlName).refresh();
    }

    const ProjectCodeBehavior = async (fieldValue) => {
        if (fieldValue) {
            fd.control('Form_Start_Finish_Detail').buttons[0].disabled = false;
            ...
            localStorage.setItem('Form_Start_Finish/JobCode', JSON.stringify(fieldValue.LookupValue));
        } else {
            fd.control('Form_Start_Finish_Detail').buttons[0].disabled = true;

            fd.field('ProjectName').value = null;
            localStorage.setItem('Form_Start_Finish/JobCode', JSON.stringify(null));
        }

        filterDataTable("Form_Start_Finish_Detail");
    }
    ...
    /**************************ONCHANGE**************************/
    fd.control('Form_Start_Finish_Detail').dialogOptions = {
        width: 1280,
        height: 720
    }
