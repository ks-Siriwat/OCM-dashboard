...
fd.spRendered(async function () {
    ...
    (async function () {
        let jobCode = transformLocalStorageValue(localStorage.getItem('Form_Start_Finish/JobCode'));
        if (!jobCode) {
            console.log('No job code');
            return false;
        }
        fd.field('SF_JobCode').value = jobCode;

        let newOrdinalNum;
        let newUpdateCount;
        await getItemsFrom('Form_Start_Finish_Detail', `SF_JobCode eq '${jobCode}'`)
            .then(res => {
                if (res.length === 0) {
                    newOrdinalNum = 1;
                    newUpdateCount = 1;
                    ...
                    let sortedUpdateCount = res.map(e => e.SF_UpdateCount).sort((a, b) => a - b);
                    newOrdinalNum = sortedOrdinalNum.pop() + 1;
                    newUpdateCount = sortedUpdateCount.pop() + 1;
                }
                fd.field('SF_OrdinalNum').value = newOrdinalNum;
                fd.field('SF_UpdateCount').value = newUpdateCount;
            })
    })()
